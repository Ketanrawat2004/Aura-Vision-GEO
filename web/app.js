...
+}`;
  ...
  if (btnExportJson) {
    btnExportJson.addEventListener('click', () => {
      if (!currentReport) {
        showToast('No active audit report to export');
        return;
      }
      const clean = sanitize(currentReport.site);
      const blob = new Blob([JSON.stringify(currentReport, null, 2)], { type: 'application/json' });
      downloadFile(blob, `auravision_geo_${clean}.json`);
  ...
  if (btnExportMd) {
    btnExportMd.addEventListener('click', () => {
      ...
      const clean = sanitize(currentReport.site);
      const mdContent = generateMarkdownReport(currentReport);
      const blob = new Blob([mdContent], { type: 'text/markdown' });
      downloadFile(blob, `auravision_geo_${clean}.md`);
      showToast('Executive Markdown report downloaded');
    });
  }

  // Copy Executive Report Handler
  const btnCopyReport = document.getElementById('btn-copy-report');
  if (btnCopyReport) {
    btnCopyReport.addEventListener('click', () => {
      if (!currentReport) {
        showToast('No active audit report to copy');
        return;
      }
      const s = currentReport.summary || {};
      const copyStr = `AuraVision GEO™ Executive Audit: ${currentReport.site}\nScore: ${s.overall_score}/100 (Grade ${s.grade})\nFindings: ${s.total_findings} (${s.critical} Critical, ${s.high} High, ${s.medium} Medium)\nProtocol: ${currentReport.verification ? currentReport.verification.proof_hash : 'SHA-256 Ledger Verified'}\nVerified via agentskills.io Enterprise Standard.`;
      copyTextToClipboard(copyStr, 'Executive audit summary copied to clipboard');
    });
  }
  ...
  function generateMarkdownReport(report) {
    ...
    const dateStr = new Date(report.audited_at || Date.now()).toUTCString();
    let md = `# AuraVision GEO™ Executive Audit Report\n\n`;
    md += `**Target Site:** ${report.site}  \n`;
    md += `**Audited At:** ${dateStr}  \n`;
    md += `**Overall GEO Score:** ${s.overall_score}/100 (Grade ${s.grade})  \n`;
    md += `**Cryptographic Proof:** \`${report.verification ? report.verification.proof_hash : 'SHA-256 Verified'}\`  \n\n`;
    md += `---\n\n## Executive Scorecard Summary\n`;
    md += `- **Critical Visibility Barriers:** ${s.critical}\n`;
    md += `- **High Severity Friction Points:** ${s.high}\n`;
    md += `- **Medium Severity Enhancements:** ${s.medium}\n`;
    md += `- **Total Diagnostic Findings:** ${s.total_findings}\n\n`;
    md += `---\n\n## Actionable Diagnostic Findings\n\n`;
    findings.forEach((f, idx) => {
      md += `### ${idx + 1}. [${f.severity.toUpperCase()}] ${f.title}\n`;
      md += `* **Category:** ${f.category} / ${f.subcategory}\n`;
      md += `* **Evidence:** ${f.evidence}\n`;
      if (f.suggested_action) {
        md += `* **Suggested Action:** ${f.suggested_action.summary}\n`;
      }
      md += `\n`;
    });
    md += `---\n*Generated by AuraVision GEO™ — Adhering to agentskills.io Standard.*\n`;
    return md;
  }
  ...
  function renderRadarChart(report, isUnreachable, pillarScores) {
    ...
    if (isUnreachable) {
      sitePoly.setAttribute('points', '160,160 160,160 160,160 160,160 160,160');
      for (let i = 0; i < 5; i++) {
    ...
    for (let i = 0; i < 5; i++) {
      const sVal = Math.max(15, Math.min(100, siteVals[i]));
      const bVal = Math.max(15, Math.min(100, benchVals[i]));

      const sR = (sVal / 100) * maxR;
      const bR = (bVal / 100) * maxR;

      const sx = Math.round(cx + sR * Math.cos(angles[i]));
      const sy = Math.round(cy + sR * Math.sin(angles[i]));

      const bx = Math.round(cx + bR * Math.cos(angles[i]));
      const by = Math.round(cy + bR * Math.sin(angles[i]));

      sitePoints.push(`${sx},${sy}`);
    ...
    sitePoly.setAttribute('points', sitePoints.join(' '));
    benchPoly.setAttribute('points', benchPoints.join(' '));
  }

  function renderReadinessMatrix(report, isUnreachable) {
    const grid = document.getElementById('readiness-grid');
    if (!grid) return;

    if (isUnreachable) {
      grid.innerHTML = '<div style="grid-column: 1 / -1; padding: 1rem; color: var(--severity-critical); font-size: 0.85rem; font-weight: 600;">Site is unreachable. All AI search engines receive connection timeouts or DNS lookup failures.</div>';
      return;
    }

    const findings = report.findings || [];
    const isBotBlocked = (botName) => findings.some(f => 
      (f.subcategory === 'crawlability' || f.title.toLowerCase().includes('robots.txt')) &&
      f.evidence && f.evidence.toLowerCase().includes(botName)
    );
    const hasSitewideBlock = findings.some(f => 
      f.title.toLowerCase().includes('blocks known ai') || 
      (f.evidence && f.evidence.toLowerCase().includes('disallow: /') && !f.evidence.toLowerCase().includes('allow'))
    );
    const hasSchemaMissing = findings.some(f => f.subcategory === 'structured-data' && (f.title.toLowerCase().includes('product') || f.title.toLowerCase().includes('schema') || f.title.toLowerCase().includes('implies')));
    const hasHydrationGap = findings.some(f => f.subcategory === 'rendering' || f.title.toLowerCase().includes('hydration') || f.title.toLowerCase().includes('spa'));
    const hasEntityUnanchored = findings.some(f => f.subcategory === 'attribution' || f.title.toLowerCase().includes('entity') || f.title.toLowerCase().includes('disambiguation') || f.title.toLowerCase().includes('sameas'));

    const engines = [
      {
        name: 'ChatGPT & SearchGPT',
        icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M22.28 9.38a5.57 5.57 0 0 0-.46-4.3 5.68 5.68 0 0 0-5.32-2.88 5.56 5.56 0 0 0-4.08-1.78 5.68 5.68 0 0 0-5.4 3.92 5.57 5.57 0 0 0-3.9 2.82 5.68 5.68 0 0 0 .74 6 5.57 5.57 0 0 0 .46 4.3 5.68 5.68 0 0 0 5.32 2.88 5.56 5.56 0 0 0 4.08 1.78 5.68 5.68 0 0 0 5.4-3.92 5.57 5.57 0 0 0 3.9-2.82 5.68 5.68 0 0 0-.74-6z"/></svg>',
        status: (hasSitewideBlock || isBotBlocked('gptbot')) ? 'blocked' : (hasSchemaMissing ? 'at-risk' : 'admitted'),
        statusLabel: (hasSitewideBlock || isBotBlocked('gptbot')) ? 'BLOCKED SITEWIDE' : (hasSchemaMissing ? 'PARTIAL / AT RISK' : 'ADMITTED & GROUNDED'),
        reason: (hasSitewideBlock || isBotBlocked('gptbot')) 
          ? 'GPTBot is disallowed in robots.txt. ChatGPT cannot browse live pages or ground citations.'
          : (hasSchemaMissing 
            ? 'Admitted, but missing JSON-LD Product/Pricing schema forces ChatGPT to regex-parse free text.'
            : 'GPTBot crawl permitted. Clean structured data enables factual answer grounding.')
      },
      {
        ...
        status: (hasSitewideBlock || isBotBlocked('perplexitybot')) ? 'blocked' : 'admitted',
        statusLabel: (hasSitewideBlock || isBotBlocked('perplexitybot')) ? 'BLOCKED SITEWIDE' : 'ADMITTED & GROUNDED',
        reason: (hasSitewideBlock || isBotBlocked('perplexitybot'))
          ? 'PerplexityBot is disallowed. Perplexity cannot index real-time pages for search citations.'
          : 'Perplexity indexing active. Generates verifiable citation pills with canonical URLs.'
      },
      {
        ...
        status: isBotBlocked('google-extended') ? 'blocked' : (hasHydrationGap ? 'at-risk' : 'admitted'),
        statusLabel: isBotBlocked('google-extended') ? 'OPTED OUT' : (hasHydrationGap ? 'PARTIAL / AT RISK' : 'ADMITTED & GROUNDED'),
        reason: isBotBlocked('google-extended')
          ? 'Google-Extended opt-out signal active. Excluded from Gemini RAG training and AI Overviews.'
          : (hasHydrationGap
            ? 'Allowed, but DOM hydration gap causes crawler to encounter empty SPA template shells.'
            : 'Admitted. Content eligible for Google AI Overviews synthesized direct answers.')
      },
      {
        name: 'Claude Web Retrieval',
        icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="12" r="9"/></svg>',
        status: (hasSitewideBlock || isBotBlocked('claudebot')) ? 'blocked' : 'admitted',
        statusLabel: (hasSitewideBlock || isBotBlocked('claudebot')) ? 'BLOCKED SITEWIDE' : 'ADMITTED & GROUNDED',
        reason: (hasSitewideBlock || isBotBlocked('claudebot'))
          ? 'ClaudeBot is disallowed in robots.txt. Claude cannot perform live browser lookups on domain.'
          : 'Claude web retrieval permitted. Server-rendered semantic HTML readable without execution.'
      },
      {
        name: 'Apple Intelligence & Siri',
        icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20.94c1.5 0 2.75 1.06 4 1.06 3 0 6-8 6-12.22A4.91 4.91 0 0 0 17 5c-2.22 0-4 1.44-5 2-1-.56-2.78-2-5-2a4.9 4.9 0 0 0-5 4.78C2 14 5 22 8 22c1.25 0 2.5-1.06 4-1.06Z"/><path d="M10 2c1 .5 2 2 2 5"/></svg>',
        status: isBotBlocked('applebot') ? 'blocked' : (hasEntityUnanchored ? 'at-risk' : 'admitted'),
        statusLabel: isBotBlocked('applebot') ? 'BLOCKED' : (hasEntityUnanchored ? 'PARTIAL / AT RISK' : 'ADMITTED & GROUNDED'),
        reason: isBotBlocked('applebot')
          ? 'Applebot-Extended disallowed. Excluded from iOS 18+ Siri and Spotlight semantic search.'
          : (hasEntityUnanchored
            ? 'Allowed, but missing authoritative sameAs entity graph leads to entity brand confusion.'
            : 'Applebot permitted. Siri and Spotlight can index official brand authority metadata.')
      },
      {
        name: 'DeepSeek Reasoning & Web',
        ...
        status: (hasSitewideBlock || isBotBlocked('bytespider')) ? 'blocked' : 'admitted',
        statusLabel: (hasSitewideBlock || isBotBlocked('bytespider')) ? 'BLOCKED SITEWIDE' : 'ADMITTED & GROUNDED',
        reason: (hasSitewideBlock || isBotBlocked('bytespider'))
          ? 'Crawler blocked by User-Agent wildcard. DeepSeek reasoning engine cannot fetch live facts.'
          : 'Standard crawlers permitted. DeepSeek can extract clean markdown facts and API endpoints.'
      }
    ];

    grid.innerHTML = engines.map(eng => `
      <div class="readiness-card">
        <div class="readiness-engine-header">
          <span class="readiness-engine-name">${eng.icon} ${eng.name}</span>
          <span class="readiness-status-pill ${eng.status}">${eng.statusLabel}</span>
        </div>
        <p class="readiness-reason">${eng.reason}</p>
      </div>
    `).join('');
  }
